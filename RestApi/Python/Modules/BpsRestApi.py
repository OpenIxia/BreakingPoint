...
#Get Run time stats            
    ...
    def getRTSbyGroup(self, runid, statsGroup, enableRequestPrints = False):
        service = 'https://' + self.ipstr + '/api/v1/bps/tests/operations/getRTS'
        jheaders = {'content-type': 'application/json'}
        jdata = json.dumps({'runid':runid, 'statsGroup':statsGroup})
        r = self.session.post(service, data=jdata, headers=jheaders, verify=False)
        if(enableRequestPrints):
            self.pretty_print_requests(r)
        if(r.status_code == 200):
...
# Network Neighbourhood   
 ...
 # Component modification     
    ...
    def uploadCapture(self, filePath, force, enableRequestPrints = False):
        service = 'https://' + self.ipstr + '/api/v1/bps/upload/capture'
        fileName = basename(filePath)
        files = {'force': force, 'file': (fileName, open(filePath, 'rb'), 'multipart/form-data')}
    ...
    def saveNormalTest(self, enableRequestPrints=False):
...
#Export Report
    def exportTestReport(self, testId, reportName, location, enableRequestPrints = False):
        ...
        service = 'https://' + self.ipstr + '/bps/api/v1/bps/export/report/' + testId + '/' + reportFormat
...
#Cloud updates
    def checkCloudUpdates(self, enableRequestPrints = False):
        service  = 'https://' + self.ipstr + '/bps/api/v1/admin/cloudupdates/operations/check'
        jheaders = {'content-type': 'application/json'}
        r = self.session.post(service, headers=jheaders, verify=False)
        if(enableRequestPrints):
            self.pretty_print_requests(r)
        checkId = r.json().get('id')
        print ("CloudUpdates CheckId is %s" %checkId)
        return checkId

    def checkCloudUpdatesProgress(self, checkId, enableRequestPrints = False):
        service  = 'https://' + self.ipstr + '/bps/api/v1/admin/cloudupdates/operations/check/' + str(checkId)
        r = self.session.get(service, verify=False)
        if(enableRequestPrints):
            self.pretty_print_requests(r)
    ...
    def checkAvailablePackages(self, enableRequestPrints = False):
    ...
    def installDailyMalware(self, package, enableRequestPrints = False):
        service  = 'https://' + self.ipstr + '/bps/api/v1/admin/cloudupdates/operations/install'
        jheaders = {'content-type': 'application/json'}
        jdata = json.dumps(package)
        print (jdata)
        r = self.session.post(service, data=jdata, headers=jheaders, verify=False)
        if(enableRequestPrints):
            self.pretty_print_requests(r)
        if(r.status_code == 202):
            print ('Daily malware %s installation has started...' %package[0].get("versionString"))
    ...
    def uninstallDailyMalware(self, package2, enableRequestPrints = False):
        service  = 'https://' + self.ipstr + '/bps/api/v1/admin/cloudupdates/installedpackages/4/versions/operations/remove'
        jheaders = {'content-type': 'application/json'}
        jdata = json.dumps(package2)
        print (jdata)
        r = self.session.post(service, data=jdata, headers=jheaders, verify=False)
        if(enableRequestPrints):
            self.pretty_print_requests(r)
        if(r.status_code == 202):
            print ('Daily malware %s uninstallation has started.' %package2.get("versions")[0].get("version"))
        return r.json().get('id')