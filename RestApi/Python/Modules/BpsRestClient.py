...
class BPS:
    ...
    def portsStateJson(self, enableRequestPrints=False):
        ...
        if (r.status_code == 200):
            print(r.json().get('portReservationState'))
            return r.json().get('portReservationState')
    ...
    def modifyNormalTest2(self, componentId, elementId, paramId, Value, enableRequestPrints=False):
        ...
        jheaders = {'content-type': 'application/json'}
        jdata = json.dumps(
            {'newParams': {'componentId': componentId, 'elementId': elementId, 'paramId': paramId, 'value': Value}})
        print(jdata)
        r = self.session.patch(service, data=jdata, headers=jheaders, verify=False)
        if (enableRequestPrints):
            self.pretty_print_requests(r)
    ...
    def modifyMulticast(self, elementId, paramId, Value, enableRequestPrints=False):
        ...
        source = ['10.1.1.3']
        jdata = json.dumps(
            {'multicastNewParams': {'elementId': elementId, 'paramId': paramId, 'value': Value, "sourceIp": source}})
        print(jdata)
        r = self.session.patch(service, data=jdata, headers=jheaders, verify=False)
        if (enableRequestPrints):
            self.pretty_print_requests(r)
...
class StatProxy:
    ...
    def GetTestProgress(self, testid):
        response = self.BPSProxy.getTestProgress(testid)
        return response

    def GetRealTimeStats(self, testid):
        response = self.BPSProxy.getRealTimeStatistics(testid)
        if StatProxy._is_json(response):
            return json.loads(response).get('rts')
        else:
            return response

    def GetStat(self, statname, testid):
        response = self.BPSProxy.getRealTimeStatistics(testid)
        if StatProxy._is_json(response):
            values = json.loads(response).get('values')
            result = values.get(statname)
            return result
        else:
            return response

    def GetTestComponents(self, testname):
        response = self.BPSProxy.testComponents(testname)
        return response.get('result')

    def GetComponentStats(self, testid, componentid):
        response = self.BPSProxy.compStats(testid, componentid)
        return response

    def GetAggregatedStats(self, testid):
        response = self.BPSProxy.aggStats(testid)
        return response

    def GetProtocolStats(self, testid, componentid):
        response = self.BPSProxy.protoStats(testid, componentid)
        return response

    def AddStatWatch(self, statname):
        self.StatCriteria.append(statname)

    def RemoveStatWatch(self, statname):
        if statname in self.StatCriteria:
            self.StatCriteria.remove(statname)

    def GetRegisteredStats(self, testid):
        valueDict = {}
        for criteria in self.StatCriteria:
            value = self.GetStat(criteria, testid)
            valueDict[criteria] = value
