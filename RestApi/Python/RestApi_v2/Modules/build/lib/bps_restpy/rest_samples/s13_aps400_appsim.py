"""
Title:  Python Script Sample To Run an AppSim Test.
Actions:
    1.  Login to BPS box
    2.  Create new test
    3.  Add components
...
value = bps.testmodel.add(
    name='AppSim', type="appsim", active=True, component="appsim")
value = bps.testmodel.add(
    name='AppSim_2', type="appsim", active=True, component="appsim")

# Modify Component Values:
# Set Application Profile to "Application Traffic Mix 2016"
bps.testmodel.component['appsim_1'].set(
    {'profile': 'BreakingPoint Application Traffic Mix 2016'})
# Set Application Profile to "Top Five Streaming Apps 2016"
bps.testmodel.component['appsim_2'].set(
    {'profile': 'BreakingPoint Top Five Streaming Apps 2016'})

# Update Description
bps.testmodel.component['appsim_1'].set(
    {'description': 'Sample API Script for App Sim'})
bps.testmodel.component['appsim_2'].set(
    {'description': 'Sample API Script for App Sim'})

# Duplicate or "Clone" Component
bps.testmodel.clone(template='appsim_1', type='appsim', active=True)
bps.testmodel.clone(template='appsim_2', type='appsim', active=True)

# Change name for the cloned componets:
bps.testmodel.component["appsim_3"].set({"label": "AppSim_3"})
bps.testmodel.component["appsim_4"].set({"label": "AppSim_4"})
...
for r in l47_resource:
    bps.topology.reserveResource(group = GROUP_NUMBER, resourceId = r, resourceType = "l47")
########################################
...
print("\nReleasing resources...")
for r in l47_resource:
    bps.topology.releaseResource(group = GROUP_NUMBER, resourceId = r, resourceType = "l47")
########################################
